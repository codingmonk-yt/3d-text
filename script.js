...
class Word {
  ...
  init() {
    ...
    this.letters = letterStrings.map(
      (letter, i) =>
        new Letter(letter, this.layers, this.element, i, letterStrings.length)
    );
...
const animations = {
  simple: (word) => {
    gsap.fromTo(
      word.element,
      { rotateY: -40 },
      { rotateY: 40, ease: "power2.inOut", yoyo: true, repeat: -1, duration: 5 }
    );
  },
  intro: (word) => {
    ...
    word.letters.forEach((letter, i) => {
      letter.elements.forEach((layer, j) => {
        tl.fromTo(
          layer,
          { "--depth": 1 },
          { "--depth": 150, ease: "back", duration: 0.5 },
          0.1 * i
        );
      });
    });
  ...
  hulk: (word) => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      gsap.set(particle, {
        ...
        xPercent: 0,
        yPercent: 0,
        x: size.width * 0.15 + Math.random() * (size.width * 0.6),
        y: size.height * 0.5 - 25,
        opacity: 0,
      });
    }
    ...
    const shake = () => {
      gsap.to(word.element.parentNode, {
        keyframes: [{ x: -75 }, { x: 0 }],
        ease: "elastic",
        duration: 0.5,
      });
  ...
  curve: (word) => {
    gsap.fromTo(
      word.element,
      { rotateY: 40, rotateZ: 20, rotateX: 20, xPercent: 25 },
      {
        rotateY: -40,
        rotateZ: -20,
        rotateX: 0,
...
const words = [...document.querySelectorAll(".word")].map(
  (element) => new Word(element)
);
